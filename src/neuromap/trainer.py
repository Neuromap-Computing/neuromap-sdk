...
class TrainHistory:
    ...
    Each element in :attr:`epochs` is a dictionary with at least
    ``epoch``, ``train_loss``, and ``learning_rate`` keys.  When
    validation is used, ``val_loss`` is also present.
    """
...
class Trainer:
    ...
    Args:
        ...
        optimizer: Optimizer class name: ``"adam"`` or ``"sgd"``.
        weight_decay: L2 regularisation weight.
        scheduler: LR scheduler type: ``"plateau"``, ``"cosine"``, or
            ``"none"``.
        scheduler_factor: Factor by which LR is reduced (plateau).
        scheduler_patience: Epochs of no improvement before LR reduction.
        min_lr: Minimum learning rate for the scheduler.
        lr_warmup_epochs: Epochs for linear LR warmup from 0 to *lr*.
        ...
        grad_clip_norm: Max gradient norm for clipping (<=0 disables).
        loss_fn: Optional custom loss ``(preds, targets) -> scalar``.
            Defaults to MSE.
        use_sequence_forward: If ``True``, use ``model.forward_sequence``
            for frame-level loss instead of rate-coded ``model.forward``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        from neuromap.network import Network  # deferred to avoid cycle

        if not isinstance(network, Network):
            raise TypeError("Trainer expects a neuromap.Network instance.")
    ...
    def fit(
        ...
        *,
        ...
        Returns:
            A :class:`TrainHistory` with per-epoch metrics.
        """
